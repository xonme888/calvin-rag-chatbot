...
from chatbot.application.bootstrap import build_default_orchestrator
...
async def chat_v2(
    ...
    state = to_state(req=req, trace_id=trace_id)
    start = time.time()
    try:
...
async def chat_v2_stream(
    ...
    return EventSourceResponse(
        _stream_events(req=req, trace_id=trace_id),
        headers={
...
async def _stream_events(*, req: ChatRequest, trace_id: str) -> Any:
