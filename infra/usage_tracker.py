...
설계 결정:
- ``SessionStats`` 와 ``UsageTracker`` 를 분리. Stats는 streamlit session_state 에 영속,
  Tracker는 query 호출마다 주입(stateless에 가까움 — Stats 참조만)
...
class SessionStats:
    ...
    streamlit ``st.session_state`` 에 단일 인스턴스로 영속한다.
    """
